...
int main(int argc, char *argv[])
{
    ...
    char str[5];      // stores input from buffer
    uint ctr = 0;     // for accessing str[]
    char curr;        // for appending str[]
    uint sTime, currentTime;
    ...
    for (int i = 0; i < numProcs; i++) {
        ...
        //printf(2, "currentTime %d\n", currentTime);     // DEBUGGING
        uint sleepTime = currentTime - sTime;
        //printf(2, "time passed is %d\n", sleepTime);
        //printf(2, "arrival time is %d\n", arrivalTime[i]);
        
        if (arrivalTime[i] <= sleepTime) {
            //printf(2, "about to sleep\n");
    ...
    #else
    #ifdef CFS
        printf(1, "Scheduler Policy: Completely Fair Scheduler\n");
    #else
    #ifdef FCFS
        printf(1, "Scheduler Policy: First Come First Serve Scheduler\n");
    #endif
    #endif
    #endif
